    ...
    namespace stream {
        namespace secureudpecho {
            namespace server {
                ...
                void Server::Start (
                    ...
                    if (done) {
                        if (!path.empty ()) {
                            ...
                            THEKOGANS_UTIL_THROW_STRING_EXCEPTION (
                                "%s", "Must supply path for ServerSecureUDPSocket configuration.");
                        }
                    }
                    else {
                ...
                void Server::HandleServerSecureUDPSocketConnection (
                    ...
                    THEKOGANS_UTIL_TRY {
                        ...
                        connection->SessionAccept (
                            serverSecureUDPSocket.ctx.get (),
                            serverSecureUDPSocket.sessionInfo);
                    }
                    THEKOGANS_UTIL_CATCH_AND_LOG
                }
                ...
                void Server::HandleSecureUDPSocketHandshakeCompleted (
                        stream::SecureUDPSocket &secureUDPSocket) throw () {
                    THEKOGANS_UTIL_LOG_INFO (
                        "Session openned (%s).\n",
                        secureUDPSocket.IsSessionReused () ? "reused" : "new");
                }

                void Server::HandleSecureUDPSocketShutdownCompleted (
                        stream::SecureUDPSocket &secureUDPSocket) throw () {
                    THEKOGANS_UTIL_LOG_INFO ("%s\n", "Session closed.");
                }
